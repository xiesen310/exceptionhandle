...
import com.zork.exceptionhandle.utils.{ReadConfig, ReadHDFS, RedisUtil}
...
object KcbpExceptionHandle {
  ...
  val highHrequency: Long = threshold.highHrequency
  val timeslot = threshold.timeslot
  ...
  def main(args: Array[String]): Unit = {
    ...
    val partitionSet = message.mapPartitions(itr => {
      ...
      x.foreach(pair => {
        ...
        val k = timeSplit(0) + "-" + cid
        val timeSection = (timeSplit(1).charAt(0).toString + timeSplit(1).charAt(1).toString).toInt

        timeslot.split("-")(0)
        // 异常交易时间段
        /*if(timeSection > timeslot.split("-")(0).toInt && timeSection < timeslot.split("-")(1).toInt) {
          jedis.hmset(k,map)
        }*/

        // 高频交易
//        if (count > highHrequency) {
